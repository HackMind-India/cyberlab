...
function normalizeBatch(b){
  ...
  return qs.map(q=>({
    ...
    question_hi:q.question_hi||q.hindi||q.hi||"",
    question_en:q.question_en||q.english||q.en||"",
    options:Array.isArray(q.options)?q.options:(Array.isArray(q.choices)?q.choices:[q.option1,q.option2,q.option3,q.option4].filter(x=>x!==undefined)),
    answer:q.answer??q.correct??q.correctAnswer??q.correct_option??0,
    explanation:q.explanation||q.solution||"",
    explanation_hi:q.explanation_hi||q.solution_hi||"",
    explanation_en:q.explanation_en||q.solution_en||""
  }));
}
function qText(q){
...
async function boot(){
  try{
    ...
    D=await r.json(); if(!Array.isArray(D.batches)) D.batches=[];
    home();
